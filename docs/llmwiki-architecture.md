...
- **网页端点信任模型**：`/webui/chat`、`/dashboard` 等页面继承「能访问到 127.0.0.1:8000 即已过身份」的信任
  （网关层决定），套件内部**不做登录态/会话保护**；路由收敛后 `/api/*` 归机器接口、`/webui/*` 归人用页面，若网关暴露公网，网关必须挡 `/api/*` 放行 `/webui/*`。
- **网关令牌**：`/api/chat`、`/api/recall`（含历史别名 `/chat`、`/recall`）受 `LLM_WIKI_BRIDGE_TOKEN` 保护；服务默认绑 `127.0.0.1`，仅企业微信回调经内网穿透面向公网（沿用第二次评审 R3）。
- **凭证全部走环境变量**，绝不硬编码（`LLM_WIKI_*` 前缀下：`API_KEY` / `BRIDGE_TOKEN` / `WECOM_*` / `ILINK_*`）。
- **iLink 无需公网**：客户端主动出网长轮询，家庭宽带即可；企业微信回调才需公网/穿透。
- **离线可跑**：LLM 未配置时 `/api/chat` 降级返回检索片段预览；iLink 无 token 时后台空闲不触网。
